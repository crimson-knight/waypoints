...
on the app doesn't require it. See `docs/TESTED_ENVIRONMENT.md` for exact
versions (and `main`'s README for the full stock-vs-fork framing — this
branch intentionally ships without its own README; it exists to carry the
`notes_embedding` migration + memories for the reviewer-workflow story
documented there).
